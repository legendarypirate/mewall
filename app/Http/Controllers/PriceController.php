...
class PriceController extends Controller
{
    ...
    public function index(){
        return view('admin.price.priceEntry');
    }


    public function save(Request $request){
        $wallEntry = new Price();
        $wallEntry -> price = $request->price;
       ...
       return redirect('/price/manage')->with('message','Амжилттай');
    }
    ...
    public function manage(){
        $user = Price::all();
        return view('admin.price.priceManage', ['user'=>$user]);
    }
    ...
    public function edit($id){
        ...
        $loanEdit = Price::where('id', $id)->first();
        return view('admin.price.priceEdit', ['loan'=>$loanEdit]);

    }

    public function update(Request $request){

        $lead= Price::where('id',$request->pid)->first();
        $p1=$lead -> price ;
       $user=User::All();
        foreach($user as $users){
            ...
            $users->quarterly=0;
            if($users->active==1){
            $users->pricechangeamount+=round((strtotime(Carbon::now())-strtotime($users->runtime))/5)*$p1;
            }
            $users->runtime=Carbon::now();

            $users->save();
        }
        $lead= Price::find($request->pid);
        $lead -> price = $request->price;
        $lead -> save();
       return redirect('/price/manage')->with('message','updated');
    }
